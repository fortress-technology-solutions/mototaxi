...
describe('The AWS Event Emitter', () => {
  describe('when listening for domain events in the event queue', () => {
    ...
    describe('with successful message with matching transaction', () => {
      ...
      const sqs = {
        receiveMessage: jest.fn((config, callback) =>
          callback(null, {
            Messages: [
              {
      ...
      const eventEmitter = new AwsEventEmitter(
        sqs as any,
        config as any,
        logger
      );
      ...
      it('should process the domain event', () => {
        expect(whenMessageReceived.mock.calls[0][0]).toEqual(
          transaction.payload
        );
      });
  ...
  describe('when emitting a command to the command queue', () => {
    ...
    describe('with success', () => {
      ...
      it('should go with a transaction id', () => {
        expect(
          JSON.parse(sqs.sendMessage.mock.calls[0][0].MessageBody).transactionId
        ).not.toBeFalsy();
      });
      it('should add the command as the payload', () => {
        expect(
          JSON.parse(sqs.sendMessage.mock.calls[0][0].MessageBody).payload
        ).toEqual(command);
      });
      it('should direct the command to the right queue', () => {
        expect(sqs.sendMessage.mock.calls[0][0].QueueUrl).toBe(
          config.commandQueueUrl
        );
      });
      it('should log the transaction with the id', () => {
        const transactionId = JSON.parse(
          sqs.sendMessage.mock.calls[0][0].MessageBody
        ).transactionId;
